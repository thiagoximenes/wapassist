...
```
GitHub (itaflix-api)          →  Render (backend)
    git push main                  redeploy automático
                                   URL: https://itaflix-api.onrender.com

GitHub (itaflix-dashboard)    →  Vercel (frontend)
    git push main                  redeploy automático
                                   URL: https://admin.itaflix.com.br
```
...
Criar dois repositórios separados no GitHub: `itaflix-api` e `itaflix-dashboard`.
...
git init
git add .
git commit -m "feat: itaflix api inicial"
git remote add origin https://github.com/SEU_USUARIO/itaflix-api.git
...
git init
git add .
git commit -m "feat: itaflix dashboard inicial"
git remote add origin https://github.com/SEU_USUARIO/itaflix-dashboard.git
git push -u origin main
```

### Checklist

- [ ] Repositório `itaflix-api` criado no GitHub e código commitado
- [ ] Repositório `itaflix-dashboard` criado no GitHub e código commitado
...
3. Conecte ao repositório `itaflix-api` do GitHub
4. Configure:
...
8. Copie a URL pública gerada (ex: `https://itaflix-api.onrender.com`)
9. Cadastre essa URL como webhook no Mercado Pago:
   `https://itaflix-api.onrender.com/api/webhook/mercadopago`

### Verificar se o backend está funcionando

```bash
curl https://itaflix-api.onrender.com/health
...
- [ ] Conta Render criada
- [ ] Web Service criado e conectado ao repositório `itaflix-api`
- [ ] Build Command: `npm install && npx prisma generate`
- [ ] Start Command: `node src/server.js`
...
3. Importe o repositório `itaflix-dashboard` do GitHub
...
8. (Opcional) Em **Settings > Domains**, adicione `admin.itaflix.com.br`
...
|---|---|
| Monitor Type | HTTP(S) |
| Friendly Name | Itaflix API |
| URL | `https://itaflix-api.onrender.com/health` |
...
- [ ] Monitor HTTP configurado para `https://itaflix-api.onrender.com/health`
...
- [ ] Acessar `https://admin.itaflix.com.br` e fazer login
...
| Serviço | URL |
|---|---|
| Dashboard (admin) | `https://admin.itaflix.com.br` |
| Backend API | `https://itaflix-api.onrender.com` |
| Health check | `https://itaflix-api.onrender.com/health` |
| Webhook MP | `https://itaflix-api.onrender.com/api/webhook/mercadopago` |
| Evolution API | `https://api.itaflix.com.br` |
